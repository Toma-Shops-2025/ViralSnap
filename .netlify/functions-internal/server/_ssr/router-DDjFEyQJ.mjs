...
import { a as createServerFn, T as TSS_SERVER_FUNCTION, b as getServerFnById } from "./server-CauiqJuS.mjs";
import { s as supabaseAdmin } from "./client.server-U_pH-Evd.mjs";
import { c as createClient } from "../_libs/supabase__supabase-js.mjs";
import { v as verifyWebhook } from "./stripe.server-DSl7M_sI.mjs";
...
const appCss = "/assets/styles-Ko4NWBNg.css";
...
const $$splitComponentImporter$o = () => import("./welcome-DzBG4Plw.mjs");
const Route$s = createFileRoute("/welcome")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$n = () => import("./wallet-DpXvtqD-.mjs");
const Route$r = createFileRoute("/wallet")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$m = () => import("./upload-hqByh9Xk.mjs");
const Route$q = createFileRoute("/upload")({
  head: () => ({
    meta: [{
      title: "Post — ViralSnap"
    }, {
      name: "description",
      content: "Upload a vertical video to the ViralSnap feed."
    }, {
      name: "robots",
      content: "noindex"
    }]
...
const $$splitComponentImporter$l = () => import("./terms-CrwnFxMK.mjs");
...
const $$splitComponentImporter$k = () => import("./share-NCUsXdyo.mjs");
const Route$o = createFileRoute("/share")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$j = () => import("./settings-BdG5sqys.mjs");
...
const $$splitComponentImporter$i = () => import("./search-DjlKaejr.mjs");
...
const $$splitComponentImporter$h = () => import("./refunds-DVC56oAr.mjs");
...
const $$splitComponentImporter$g = () => import("./privacy-CR5qHJ2T.mjs");
...
const $$splitComponentImporter$f = () => import("./guidelines-D-5faCxR.mjs");
const Route$j = createFileRoute("/guidelines")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$e = () => import("./earnings-ZvGUcjhv.mjs");
const Route$i = createFileRoute("/earnings")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$d = () => import("./dmca-DmoEu7s6.mjs");
const Route$h = createFileRoute("/dmca")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$c = () => import("./discover-CfqV84RY.mjs");
...
const $$splitComponentImporter$b = () => import("./contact-BQY79BNL.mjs");
...
const $$splitComponentImporter$a = () => import("./child-safety-BVBaJjGL.mjs");
const Route$e = createFileRoute("/child-safety")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$9 = () => import("./campaigns-C3UyzyvQ.mjs");
const Route$d = createFileRoute("/campaigns")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$8 = () => import("./auth-ATRQ_ULX.mjs");
...
const $$splitComponentImporter$7 = () => import("./activity-CDGD47lE.mjs");
const Route$b = createFileRoute("/activity")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$6 = () => import("./account-deletion-DJgG9aj2.mjs");
...
const $$splitComponentImporter$5 = () => import("./index-CaqqRsxL.mjs");
...
const $$splitComponentImporter$4 = () => import("./live.index-BNTFJNHM.mjs");
const Route$8 = createFileRoute("/live/")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$3 = () => import("./u._username-MfdceRJE.mjs");
const Route$7 = createFileRoute("/u/$username")({
  loader: ({
    params
  }) => getPublicProfile({
    data: {
...
const $$splitComponentImporter$2 = () => import("./live._streamId-DAc9OpXV.mjs");
const Route$6 = createFileRoute("/live/$streamId")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$1 = () => import("./block._userId-DfyJZkEM.mjs");
const Route$5 = createFileRoute("/block/$userId")({
  validateSearch: (search) => ({
    username: typeof search.username === "string" ? search.username : void 0
  }),
  head: () => ({
    meta: [{
      title: "Block creator — ViralSnap"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("./report.user._userId-DdjF1X50.mjs");
const Route$4 = createFileRoute("/report/user/$userId")({
  validateSearch: (search) => ({
    username: typeof search.username === "string" ? search.username : void 0
  }),
  head: () => ({
    meta: [{
      title: "Report creator — ViralSnap"
    }]
  }),
...
async function restoreViralSnapPlayback() {
  ...
  const { data: publishedRows, error: listErr } = await supabaseAdmin.from("videos").select("id, media_url, mux_playback_id").eq("status", "published");
  if (listErr) throw new Error(listErr.message);
  const legacyIds = (publishedRows ?? []).filter((row) => {
  ...
  if (legacyIds.length > 0) {
    const { error: hideErr } = await supabaseAdmin.from("videos").update({ status: "removed" }).in("id", legacyIds);
  ...
  return {
    ...
    unpublishedLegacy,
    note: "Unpublished legacy Mux/dead-URL rows. Feed now only shows Supabase Storage MP4 uploads."
  };
}
const Route$3 = createFileRoute("/api/admin/restore-media")({
  server: {
    handlers: {
      POST: async ({ request }) => {
...
async function removeUserStorage(userId) {
  for (const bucket of ["videos", "covers", "avatars"]) {
...
async function takedownUserByUsername(username, reason = "policy violation") {
  const { data: profile, error } = await supabaseAdmin.from("profiles").select("id, username").eq("username", username.replace(/^@/, "").trim().toLowerCase()).maybeSingle();
  if (error) throw new Error(error.message);
  if (!profile) throw new Error(`User @${username} not found`);
  return takedownUserById(profile.id, reason);
}
async function takedownUserById(userId, reason = "policy violation", deleteAccount = true) {
  const { data: videos, error: listErr } = await supabaseAdmin.from("videos").select("id").eq("creator_id", userId);
  if (listErr) throw new Error(listErr.message);
  const { error: hideErr } = await supabaseAdmin.from("videos").update({ status: "removed" }).eq("creator_id", userId);
  if (hideErr) throw new Error(hideErr.message);
  const { error: banErr } = await supabaseAdmin.from("profiles").update({
    is_banned: true,
...
const Route$2 = createFileRoute("/api/admin/moderate")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const secret = process.env.MODERATION_SECRET ?? process.env.MEDIA_RESTORE_SECRET;
        if (!secret) {
        ...
        try {
          ...
          const result = body.userId ? await takedownUserById(body.userId, reason) : body.username ? await takedownUserByUsername(body.username, reason) : null;
          if (!result) {
...
const WelcomeRoute = Route$s.update({
  id: "/welcome",
  path: "/welcome",
  getParentRoute: () => Route$t
});
const WalletRoute = Route$r.update({
  id: "/wallet",
  path: "/wallet",
  getParentRoute: () => Route$t
});
const UploadRoute = Route$q.update({
  id: "/upload",
  path: "/upload",
  getParentRoute: () => Route$t
});
...
const ShareRoute = Route$o.update({
  id: "/share",
  path: "/share",
  getParentRoute: () => Route$t
});
...
const GuidelinesRoute = Route$j.update({
  id: "/guidelines",
  path: "/guidelines",
  getParentRoute: () => Route$t
});
const EarningsRoute = Route$i.update({
  id: "/earnings",
  path: "/earnings",
  getParentRoute: () => Route$t
});
const DmcaRoute = Route$h.update({
  id: "/dmca",
  path: "/dmca",
  getParentRoute: () => Route$t
});
...
const ChildSafetyRoute = Route$e.update({
  id: "/child-safety",
  path: "/child-safety",
  getParentRoute: () => Route$t
});
const CampaignsRoute = Route$d.update({
  id: "/campaigns",
  path: "/campaigns",
  getParentRoute: () => Route$t
});
...
const ActivityRoute = Route$b.update({
  id: "/activity",
  path: "/activity",
  getParentRoute: () => Route$t
});
...
const LiveIndexRoute = Route$8.update({
  id: "/live/",
  path: "/live/",
  getParentRoute: () => Route$t
});
const UUsernameRoute = Route$7.update({
  id: "/u/$username",
  path: "/u/$username",
  getParentRoute: () => Route$t
});
const LiveStreamIdRoute = Route$6.update({
  id: "/live/$streamId",
  path: "/live/$streamId",
  getParentRoute: () => Route$t
});
...
const ReportUserUserIdRoute = Route$4.update({
  id: "/report/user/$userId",
  path: "/report/user/$userId",
  getParentRoute: () => Route$t
});
const ApiAdminRestoreMediaRoute = Route$3.update({
  id: "/api/admin/restore-media",
  path: "/api/admin/restore-media",
  getParentRoute: () => Route$t
});
const ApiAdminModerateRoute = Route$2.update({
